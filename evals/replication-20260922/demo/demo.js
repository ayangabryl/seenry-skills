...
function set(c,open,{focus=false,instant=false,manual=true}={}){
 ...
 const previousPhase=c.phase,previousRect=[...c.rect];cancelAnimationFrame(c.raf);c.open=open;c.closing=!open;c.menu.inert=!open;c.menu.dataset.open=String(open);c.trigger.setAttribute('aria-expanded',String(open));
...
for(const c of controls){c.open=false;c.phase=0;c.rect=[...c.anchor];c.raf=0;draw(c);
