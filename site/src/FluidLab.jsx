...
export default function FluidLab({initialFamily='Merge', navigation=true}) {
 ...
 const common={fill,blur,contrast:18,};
 async function copy(){
 const recipes={
 ...
 'Merge:Panel':'Use FluidItem effect="morph" to follow changing surface bounds. A 112px closed surface expands to 224px, with a shared 24px corner token. The application owns disclosure state and focus; use CSS for content appearance. Keep trigger and revealed panel on the same fill. Retarget current geometry on interruption.',
 ...
 'Blur:Numbers':'Keep three tabular digit slots. Overlap outgoing and incoming glyphs immediately; translate ±8px, blur 2px, opacity 0↔1 over 500ms cubic-bezier(.34,1.45,.64,1), stagger 70ms left to right. Do not wait for exit before entering. Reuse exactly two layers per column and cancel/retarget on rapid input. Announce one authoritative current value.',
 ...
 {family==='Merge'&&context==='Panel'&&<FluidSurface {...common} className="fl-panel-host"><FluidItem effect="morph"><div className={'fl-panel '+(open?'is-open':'')}><button ref={trigger} aria-expanded={open} onClick={()=>O(v=>!v)}>{open?'Close options':'Options'} <Plus size={16}/></button>{open&&<div className="fl-panel-items">{['Save reference','Add to collection','Copy link'].map(t=><button key={t} onClick={()=>{N(t+' selected — demo');close()}}>{t}</button>)}</div>}</div></FluidItem></FluidSurface>}
