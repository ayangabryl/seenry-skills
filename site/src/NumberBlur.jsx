...
function Digit({value,delay,direction,reduced}) {
 const incoming=useRef(),outgoing=useRef(),last=useRef(null),changedAt=useRef(0);
 useLayoutEffect(()=>{
  ...
  const interrupted=performance.now()-changedAt.current<640;changedAt.current=performance.now();
  // Reuse exactly two layers. Retarget from the visible incoming glyph, never queue.
  const source=Number(getComputedStyle(b).opacity)>Number(getComputedStyle(a).opacity)?b:a;
  const glyph=source.textContent;const current=getComputedStyle(source);const start={transform:current.transform,opacity:current.opacity,filter:current.filter};
  a.getAnimations().forEach(x=>x.cancel());b.getAnimations().forEach(x=>x.cancel());
  b.textContent=glyph;a.textContent=value;last.current=value;
  const options={duration:interrupted?300:500,delay:interrupted?0:delay,easing:'cubic-bezier(0.34,1.45,0.64,1)',fill:'both'};
