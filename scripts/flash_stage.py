...
def main():
    ...
    record={'model_requested':args.model,'runtime_configured_model':init.get('model'),'reported_model':result.get('model'),
            ...
            'delivery_size':{'prompt_bytes':len(prompt.encode('utf-8')),'prompt_characters':len(prompt),
                'image_attachments':0,'external_schema_bytes':0,'token_count':None,
                'scope':'Exact supplied prompt text; this adapter passes no direct image attachments or external response schema. Excludes provider tokenization, image tokens, ambient context and later tool traffic. Raw provider usage, when available, is authoritative after execution.'},
