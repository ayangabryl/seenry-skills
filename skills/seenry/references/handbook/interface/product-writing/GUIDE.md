...
How copy renders (capitalization via `text-transform`, truncation, smart punctuation) belongs to `type-system`. Error markup and announcements (`aria-invalid`, live regions) belong to `accessibility`. Room for translated strings belongs to `layout-system`.
