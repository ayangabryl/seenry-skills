...
- It sets a `component-variants` search param and reads the active variant back from it, so the URL is the source of truth and every variant a link.
...
- Switching is instant. Flipping is the run's highest-frequency action, so `control-craft`'s motion restraint gives it no transition.
