...
Every other skill here judges. This one produces candidates and hands the decision back. Reviewing existing UI is `interface-audit` and `combined-review`, and it does not pick the winner.
...
| --- | --- | --- |
| Structure | `layout-system` | Grouping, order, column count, what collapses |
| Density | `layout-system` | Spacing scale, hit areas, how much fits |
| Emphasis | `color-system` | Where filled color goes, what recedes |
| Type | `type-system` | Scale steps, weight contrast, measure |
| Voice | `product-writing` | Labels, tone, how much copy |
...
Before a variant enters the picker it clears `combined-review`'s escalation triggers. Every control has an accessible name, keyboard reaches everything a pointer does, focus is visible, nothing clips at 320px and no meaning rides on color alone.
