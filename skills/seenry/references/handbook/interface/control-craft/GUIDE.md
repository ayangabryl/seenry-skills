...
Text wrapping, font rendering, tabular numbers and text spacing belong to `type-system`. Hit areas, focus, keyboard support, ARIA and reduced motion belong to `accessibility`. Grouping, section spacing, breakpoints and spatial RTL belong to `layout-system`.
...
A `scale(0.96)` on click gives a button tactile feedback. Treat `0.96` as a trial value; preserve the project’s press token and inspect the result. Add a `static` prop to switch it off where motion would distract. See [recipes for CSS, Tailwind and Motion](animations.md#scale-on-press).
