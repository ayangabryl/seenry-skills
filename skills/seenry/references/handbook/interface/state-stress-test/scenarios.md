...
Breaks land in `type-system` for wrapping and truncation, `layout-system` where the container has no room and `product-writing` where the source copy is the problem.
...
Breaks land in `type-system`; spatial mirroring lands in `layout-system`.
...
Zero-item breaks land in `product-writing` for the empty state and `layout-system` for the region; the rest land in `layout-system`.
...
Breaks land in `accessibility`; purely visual state polish lands in `control-craft`.
...
Dark-mode breaks land in `color-system`; zoom and motion land in `accessibility`.
