...
It observes rather than judges. A finding here is something that visibly broke on the page, named in the vocabulary of the domain skill that owns the fix. Reviewing code against a standard is `interface-audit` and `combined-review`; exploring design alternatives is `component-variants`.

Where `component-variants` insists on the real page, this skill isolates on purpose. You are not judging how the component looks in context. You are checking whether it defends itself when the content is worst-case.
...
| --- | --- | --- |
| One unbreakable 60-character string | Overflows the card, no wrap and no truncation | `type-system` |
| Zero items | Blank region with no message | `product-writing` |
