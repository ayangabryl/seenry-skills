...
Orchestration is all it owns. Accessibility rules belong to `accessibility`, structure to `layout-system`, copy to `product-writing`, type to `type-system`, color to `color-system`, visual polish and motion to `control-craft`. Never duplicate or override their rules here.

Change-scoped review of uncommitted work, branches and pull requests belongs to `interface-audit`, which resolves the scope and classifies findings before handing the review back.
...
### 2. Send a change to `interface-audit`

A request naming a branch, pull request, commit range, or uncommitted changes is a change review, not a screen review. Say so and ask the user to run `interface-audit`, which is user-invoked and cannot be started from here.

Never resolve a change scope here. Reading a diff, classifying findings and expanding changed files to affected surfaces belong to `interface-audit`. Guess at them and the report has a scope nobody can check.

When `interface-audit` hands a review back, it supplies the change scope, a status per finding and the change-scoped report format. Severity, ranking, the cap and the verdict stay here, and all three cover `Introduced` and `Regression` only.
