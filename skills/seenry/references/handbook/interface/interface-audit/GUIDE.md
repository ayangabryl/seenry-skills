> Seenry adaptation. Read only for the current task. Project brand rules, user intent and the installed stack take precedence over recipe defaults. Numeric values and aesthetic prescriptions below are starting points, not universal requirements. Verify API/toolchain claims against the actual project. No automatic delegation, dependency installation, scope expansion or approval is implied. See [integration rules](../../INTEGRATION.md) before applying this module.
...
Scope is all it owns. Domain rules belong to the `better-*` skills. Severity, consolidation, coverage, the cap and the verdict belong to `combined-review`, which this skill hands the review to.
...
- **A whole-repository interface audit**, which is not a change review. Hand it to `combined-review` as a repository-scope review, without this skill's scope block, statuses, or pre-existing section. With no change, every finding is pre-existing and the classification says nothing.
...
Hand every finding up with its status attached and let `combined-review` apply its cap and verdict rules.
...
### 7. Hand the review to `combined-review`

Hand `combined-review` the scope block, the affected surfaces and a status on every finding. It routes to the domain skills, applies severity, consolidates, enforces the cap and issues the verdict.

If `combined-review` is unavailable, report the resolved scope and the file inventory, name it as the missing skill and stop. Do not invent a severity scale, a cap, or a verdict.
...
| The severity scale or the finding cap restated here | Defer to `combined-review` |
...
End with `Block` when any `HIGH` remains and `Approve` otherwise, leaving the remaining findings in the table as work to do. When `combined-review` is available, the severity scale and the cap come from it.
