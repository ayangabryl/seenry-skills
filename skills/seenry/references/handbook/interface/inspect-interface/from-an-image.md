...
Sample the actual pixels rather than describing what you see. Then hand the values to `color-system`:
...
- **Measure:** count characters on a full line of body copy and compare against `type-system`'s 60 to 75 range.
...
Then check the ratio `layout-system` owns, the gap between groups against the gap within one. At or above 2, space is carrying the grouping. Below that, something else is, usually a border or a background shape.
