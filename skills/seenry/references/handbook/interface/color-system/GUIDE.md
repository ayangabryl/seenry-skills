...
Contrast requirements belong to `accessibility`. Surfaces, shadows and icon color belong to `control-craft`.
...
Use a color for one purpose across the whole interface, treating anything within `15°` of hue as the same color. If the accent means interactive, that hue on static text tells users to click something that is not clickable, and an interactive element rendered neutral misleads just as badly. Color is never the only carrier of meaning, which `accessibility` owns.
