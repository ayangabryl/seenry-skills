...
The words themselves belong to `product-writing`. Semantic heading structure belongs to `accessibility`. Spatial RTL layout and logical properties belong to `layout-system`. Contrast measurement belongs to `color-system`. This skill owns how text renders, wraps and behaves in mixed-direction content.
...
When text looks low-contrast, use `color-system` to measure the rendered pair and `accessibility` to classify the requirement. Leave the colors alone unless asked.
...
Set `lang` so browsers and assistive technology pick the right pronunciation, quotes and hyphenation. Set `dir` at the document or at the content boundary where direction changes. Preserve digit order, and use `<bdi>` to isolate a mixed-direction value. Spatial mirroring and logical CSS properties belong to `layout-system`.
