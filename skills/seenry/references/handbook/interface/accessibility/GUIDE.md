...
Contrast measurement and color fixes belong to `color-system`. Text sizing and iOS input zoom belong to `type-system`. Spatial RTL layout belongs to `layout-system`.
...
Status needs a redundant cue: an icon, text, or an underline alongside the color. Work out which WCAG contrast requirement applies, then use `color-system` to measure the rendered pair. When it fails, report the pair and the requirement it misses, and leave the colors alone unless asked.
