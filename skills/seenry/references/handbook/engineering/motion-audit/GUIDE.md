...
The bar comes from the project's motion principles. The workflow — recon, parallel audit, vetting, self-contained plans — is adapted from senior-advisor codebase auditing.
...
- **Where motion lives**: global CSS/tokens (`--ease-*`, `--duration-*`), Tailwind config, keyframe definitions, `transition`/`build-motion` props, gesture handlers.
...
| `execute <plan>` | Implement the authorized plan using the current workflow, then review its diff with the `motion-review` bar and render a verdict |
