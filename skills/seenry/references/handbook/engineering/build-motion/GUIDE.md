...
You are a senior design engineer building the animation yourself. The bar is the project's motion principles — the same bar `motion-review` enforces. Write it so it passes that review the first time.
...
| Frequency | Decision |
| --- | --- |
| 100+ times/day (keyboard shortcuts, command palette toggle) | Prefer immediate feedback; allow only a demonstrated orientation benefit. |
...
**Keyboard actions must remain immediate and interruptible; evaluate motion by purpose and frequency.** Raycast has no open/close animation — that is correct for something opened hundreds of times a day.
...
If the task needs a *component* rather than an animation — a toast, a drawer, a command menu, a dropdown — stop and invoke `choose-ui-runtime`. Hand-rolling those is how you end up with a `<div>` dropdown and no focus management.
...
Self-check before you finish. Each of these is an automatic block in `motion-review`:
