...
For focused packets, optional `project.guide_topics` accepts `subject-fit` and `convergence`. These select the quality-diagnosis guide at plan, prototype, compare, review and refine; subject fit also selects the scope's concept guide while planning, prototyping or refining. Use them for an interchangeable concept or repetition across unrelated projects. They are separate from `project.decisions`, which selects rendered lessons; guide topics do not attach lesson images or change default routes.
