...
Use before expanding a new component and after a visual rejection. A working interaction and a plausible style rationale cannot pass this checkpoint.
...
A service name is not a brand system. Do not derive an entire palette or theme from a noun (a dental clinic becoming green, or a finance product becoming blue) without supplied identity or tested visual evidence. A heading should name this task; a promotional promise above a routine operation needs a reason to be there. A familiar font is not a defect: inspect actual weight, size, line length and relationships before changing families.
...
A successful subtraction can improve hierarchy without creating a strong visual idea. If the result is still interchangeable or poorly proportioned, revise that cause separately. Do not label either variant professionally validated from self-review alone.
