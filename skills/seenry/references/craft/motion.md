...
Choose a concrete local implementation before inventing another one: `motion_helpers: ["geometry"]` supplies interruptible disclosure, `"icon-swap"` supplies anchored two-state glyphs, and `"number"` supplies changing numeric values. For pricing switches, read [number transitions](../../../seenry-motion/references/number-transitions.md#pricing-and-billing-switches): a whole-price entrance is not digit-level motion. The focused packet includes selected source and runtime dependencies. Copy those files with licenses, then exercise them in the product. For menus and contextual controls use the [product transition recipes](../../../seenry-motion/references/product-transitions.md). Effects do not repair detached controls or unclear copy.
