...
ROLES = {'source-material', 'construction', 'candidate', 'reference', 'interaction'}
...
def prepare(stage, project, task, out, root=ROOT, profile='complete', research_source='local', evidence=None, evidence_root=None):
    ...
    if lesson_bundle:
        ...
        for lesson in lesson_bundle['lessons']:
            for item in lesson['evidence']:
                ...
                images.append({'source': str(source), 'role': 'reference', 'lesson': lesson['id'], 'content': content})
    ...
    manifest = {'schema':1, 'stage':stage, 'profile':profile, 'prompt_sha256':digest(prompt.encode()),
