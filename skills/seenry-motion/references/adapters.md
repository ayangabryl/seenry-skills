...
- `assets/count-pop.mjs` with `assets/count-pop.css`: an original, dependency-free per-digit entrance for a small nonnegative integer count. Read [number transitions](number-transitions.md) for its limits; use the number renderer above when rolling continuity, formatting or broader value support matters.
