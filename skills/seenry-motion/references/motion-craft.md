...
When the concept calls for a luminous boundary, a truthful process orb, liquid grouping, reflective material or image reveal, inspect [the expressive-effects reference](expressive-effects.md) before selecting the runtime. These are five possible visual mechanisms, not a required embellishment pass. Choose an implementation that fits the project's stack and test it in the actual interface.
