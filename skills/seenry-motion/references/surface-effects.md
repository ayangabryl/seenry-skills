...
Research (24 September 2026): the source study separated molten color, seam marbling and crisp faces. The earlier Seenry implementation blurred the entire source pair, which lost detail and did not model the seam. The bundled renderer is an original, smaller SVG implementation of contact-local blending, not a dependency install or a claim of pixel-equivalent reconstruction. The [local liquid package archive](../assets/optional-effects/liquid-gooey-0.2.2.tgz) preserves a published implementation for code inspection; its license is [bundled separately](../assets/optional-effects/liquid-gooey-LICENSE). The reference has additional geometry, tuning and easing; our demo remains a bounded two-image adaptation. Keep research provenance out of consumer copy prompts.
