...
The current renderer is an original bounded two-image adaptation. It separates a soft combined silhouette from crisp outer faces, with a contact mask controlled by the measured gap. The earlier implementation blurred the whole image pair and lost detail. This does not establish pixel-equivalent reconstruction of any other effect; compare the actual requested reference before claiming a match.
