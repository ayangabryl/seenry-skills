...
`assets/number-transition.mjs` exports `createNumberTransition({slot,value,locales,format,duration,reserveValues})`. The text-only `slot` should contain the usable initial value before enhancement. Keep its unit and control labels in the application. `update(value)` retargets from the current visual state; `set(value)` settles immediately; `destroy()` stops enhancement and leaves the latest formatted text. Values must be finite. The adapter exposes `value` and `animationSupported`; these are not evidence of visual quality.
